    ...
    def discover(self):
        ...
        if self.config:
            for transmitter_config in self.config:
                device_name = transmitter_config.get(CONF_DEVICE_PATH)
                ...
                if device_name:
                    ...
                    _LOGGER.info(
                        ...
                        f"'{device_name}' with serial number: '{serial_number}'"
                    )
                    ...
                    elero_transmitter = EleroTransmitter(
                        device_name, serial_number, baudrate, bytesize, parity, stopbits
                    )
                    ...
                    if elero_transmitter.get_transmitter_state():
                        ...
                        _LOGGER.error(
                            f"Failed to connect to manually specified device: '{device_name}'"
                        )
        ...
        for cp in list_ports.comports():
            ...
            if found_elero_stick or preset_elero_stick:
                ...
                # Skip if this serial number is already handled by manual configuration
                if self.config:
                    already_configured = False
                    for transmitter_config in self.config:
                        if (transmitter_config.get(CONF_TRANSMITTER_SERIAL_NUMBER) == elero_serial_number and 
                            transmitter_config.get(CONF_DEVICE_PATH)):
